                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        isMobile() ? const SizedBox.shrink() : const SizedBox(width: 24),
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 16),
                          child: Obx(() => Text('${propCtrl.searchResultCount.value} result Found',
                              style: TextStyle(
                                  fontSize: isMobile()
                                      ? 16
                                      : isDesktop()
                      ...
                      child: Row(
                        ...
                        children: [
                          Expanded(
                            ...
                            child: Obx(() => Column(
                                  children: [
                                    Visibility(visible: propCtrl.refreshCount.value == -1, child: Text(propCtrl.refreshCount.value.toString())),
                                    FutureBuilder(
                                        future: propCtrl.allPropertiesWeb.value,
                                        builder: (context, AsyncSnapshot snap) {
                                          List<Property> properties = snap.data ?? [];
                                          Future.delayed(const Duration(milliseconds: 1500), () => propCtrl.searchResultCount.value = properties.length);
                                          return snap.connectionState == ConnectionState.waiting
                                              ? SizedBox(
                                                  height: 520,
                                                  child: Center(child: Preloader.loadingWidget()),
                                                )
                                              : Obx(
                                                  () => homeCtrl.isList.value && !isTabletDown()
                                                      ? buildListContainer(properties: properties)
                                                      : PropertiesGrade(properties: properties),
                                                );
                                        }),
                                  ],
                                )),
                          ),
                          isNotDeskTop() ? const SizedBox.shrink() : const SizedBox(width: 24),
                          isNotDeskTop()
                              ? const SizedBox.shrink()
                              : Expanded(
                                  flex: 2,
                                      ...
                                      child: Stack(children: [
                                        Column(
                                          children: [
                                            ...
                                            Obx(() => propCtrl.showMoreFilter.value || true
                                                ? Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      const SizedBox(height: 8),
                                                      FutureBuilder(
                                                          future: cPropCtrl.getAmenities(all: true),
                                                          builder: (context, AsyncSnapshot snap) {
